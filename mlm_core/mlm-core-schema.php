...
function mlm_core_update_mlm_user_for_product_price()
{
    global $wpdb;
    $charset_collate = mlm_core_set_charset();
    $table_prefix = mlm_core_get_table_prefix();
    ...
    $sql[] = "ALTER TABLE   {$table_prefix}mlm_users "
            . "ADD product_price int(11) default 0 AFTER payment_status";
    if (mysql_num_fields(mysql_query("select * from {$table_prefix}mlm_users")) == 9)
        mysql_query($sql[0])or die(mysql_error() . 'ALTER');
}
...
function mlm_core_update_mlm_leftleg()
{
    ...
    if (mysql_num_fields(mysql_query("select * from {$table_prefix}mlm_leftleg")) == 5)
        mysql_query($sql[0])or die(mysql_error() . 'ALTER');
}
...
function mlm_core_update_mlm_rightleg()
{
    ...
    $sql[] = "ALTER TABLE   {$table_prefix}mlm_rightleg "
            . "ADD payout_id int(11) default 0 AFTER  commission_status";
    if (mysql_num_fields(mysql_query("select * from {$table_prefix}mlm_rightleg")) == 5)
        mysql_query($sql[0])or die(mysql_error() . 'ALTER');
}
...
function mlm_core_update_payout()
{
    ...
    $sql[] = "ALTER TABLE   {$table_prefix}mlm_payout ADD withdrawal_initiated BOOLEAN NOT NULL DEFAULT '0' COMMENT '0=>No, 1=> Yes' AFTER bonus_amount,
				ADD  withdrawal_initiated_comment VARCHAR( 200 ) NOT NULL AFTER  withdrawal_initiated,
				ADD withdrawal_initiated_date DATE NOT NULL AFTER withdrawal_initiated_comment,
				ADD payment_mode VARCHAR( 100 ) NOT NULL AFTER bonus_amount,
				ADD payment_processed BOOLEAN NOT NULL DEFAULT '0' COMMENT '0=>No, 1=> Yes' AFTER withdrawal_initiated_date,
				ADD payment_processed_date DATE NOT NULL AFTER payment_processed,
				ADD beneficiary VARCHAR( 100 ) NOT NULL AFTER payment_processed_date,
				ADD other_comments VARCHAR( 100 ) NOT NULL AFTER user_bank_account_no ,
				ADD referral_commission_amount DOUBLE( 10, 2 ) NOT NULL DEFAULT '0.00' AFTER `commission_amount`,
				ADD total_amt VARCHAR( 100 ) NOT NULL DEFAULT '0.00' AFTER bonus_amount ,
				ADD capped_amt VARCHAR( 100 ) NOT NULL DEFAULT '0.00'  AFTER total_amt ,
				ADD cap_limit VARCHAR( 100 ) NOT NULL DEFAULT '0.00' AFTER capped_amt ,{$charset_collate}";



    if (mysql_num_fields(mysql_query("select * from {$table_prefix}mlm_payout")) < 29)
        mysql_query($sql[0])or die(mysql_error() . 'ALTER');
}
...
function mlm_core_update_payout_master()
{
    ...
    if (mysql_num_fields(mysql_query("select * from {$table_prefix}mlm_payout_master")) < 3)
        mysql_query($sql[0])or die(mysql_error() . 'ALTER');
}
...
{
    $table_prefix = mlm_core_get_table_prefix();
    mysql_query("update {$table_prefix}mlm_payout set total_amt = commission_amount+referral_commission_amount+bonus_amount,
