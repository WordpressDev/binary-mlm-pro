...
//$status=" WHERE status=$epin_status";
        ...
        ?>
        <form method="post" action="<?= plugins_url() ?>/binary-mlm-pro/mlm_html/export.php">
            <input type="hidden" name ="listarray" value='<?= $value ?>' />
            <input type="submit" name="export_csv" value="<?php _e('Export to CSV', 'binary-mlm-pro'); ?>" class="button"/></form>
        <?php
    }
    else
    {
