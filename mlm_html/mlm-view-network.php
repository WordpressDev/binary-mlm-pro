...
class BinaryTree 
{
	...
	{
		$table_prefix = mlm_core_get_table_prefix();
		$sql = "SELECT user_id FROM {$table_prefix}mlm_users WHERE username = '".$username."'";
		$sql = mysql_query($sql);
		$row = mysql_fetch_array($sql);
        ...
        function totalLeftLegUsers($key)
        {
                global $wpdb;	
                $table_prefix = mlm_core_get_table_prefix();
                return $num = $wpdb->get_var(" SELECT COUNT(*) AS num FROM {$table_prefix}mlm_leftleg  WHERE pkey = '".$key."'");
        }
        ...
        function totalRightLegUsers($key)
        {
                global $wpdb;	
                $table_prefix = mlm_core_get_table_prefix();
                return $num = $wpdb->get_var(" SELECT COUNT(*) AS num FROM {$table_prefix}mlm_rightleg WHERE pkey = '".$key."'");
        }
	...
	function searchUser()
	{
		...
		if(user=="")
		{
 		...
 		<table border="0" cellspacing="0" cellpadding="0" >
			<tr>
  				<td align="center">	
					<form action="<?php bloginfo('url','binary-mlm-pro');?>/?page_id=<?=$obj->view_page_id?>&k=<?php _e($res['user_key']);?>" method="post">
						<input type="submit" value="<?= __('YOU','binary-mlm-pro'); ?>">
					</form>
				</td>
			<td align="center">
				<form name="usersearch" id="usersearch" action="" method="post" onSubmit="return searchUser();">
					<input type="text" name="username" id="username"> <input type="submit" name="search" value="<?= __('Search','binary-mlm-pro'); ?>">
				</form>
			</td>
		</tr>               
		</table>
