        ...
        <table width="100%" border="0" cellspacing="10" cellpadding="1" class="payout-summary">
            ...
            { ?>
                <tr>
                    ...
                    <td width="50%" class="right"><?= $mlm_settings['currency'] . ' ' . $detailArr['dirrefommamount'] ?></td>
                </tr>
        <?php } ?>
            <tr>
            ...
            <tr>
                ...
                <td width="50%" class="right"><?= $mlm_settings['currency'] . ' ' . $detailArr['total_amt'] ?></td>
            </tr>
            <?php if ($detailArr['total_amt'] >= $detailArr['cap_limit'] && !empty($detailArr['cap_limit']))
            {
                ...
                else
                {
                    ...
                    <tr>
                        ...
                        <td width="50%" class="right"><?= $mlm_settings['currency'] . ' ' . $detailArr['cap_limit'] ?></td>
                    </tr>
            <?php }
        }
        ?>
            ...
            <tr>
                <td width="50%"><strong><?php _e('Net Amount', 'binary-mlm-pro'); ?><?= (($detailArr['total_amt'] >= $detailArr['cap_limit'] && !empty($detailArr['cap_limit']))) ? ($detailArr['cap_limit'] == '0.00' ? '' : '(capped)') : ''; ?></strong>	</td>
                <td width="50%" class="right"><strong><?= $mlm_settings['currency'] . ' ' . $detailArr['capped_amt'] ?></strong></td>
            </tr>
            <tr>
                <td colspan="2" class="right">
                    <div id="message" style="display:none;color:#0DA443;"><?php _e
        ...
        { ?>
                        ...
                        <div id="comment_form">
                            <form name="contact" method="post" action="">
                                <fieldset>
                                    <label for="name1" id="name_label"><b><?php _e('Comment, if any', 'binary-mlm-pro'); ?>:</b>&nbsp;&nbsp;</label>
                                    <input type="text" name="name1" id="name1" size="30" value="" class="text-input"/>
                    ...
                    if (confirm("<?php _e('This would initiate a withdrawal for this payout. Would you like to proceed?', 'binary-mlm-pro') ?>")) {
                        $.ajax({
                            ...
                            success: function() {
                                ...
                                $('#message').html("<h3 class='initiatedmsg'><?= __('Your Withdrawal Request Initiated.', 'binary-mlm-pro') ?></h3>")
                                        .hide()
                                        .fadeIn(1500, function() {
                                            $('#message').append("<?= __('Thanks for Patience.', 'binary-mlm-pro') ?>");
                                        });
                            }
                        });
                        return false;
...
function paymentDeatil($memberId, $payoutId)
{
    ...
    if (isset($memberId) && isset($payoutId))
    {
        ...
        if ($detail['payment_mode'] == 'cheque')
        {
            ...
            <table><tr>
                    <th><?= __('Withdrawal Date', 'binary-mlm-pro'); ?></th>
                    <th><?= __('Payment Date', 'binary-mlm-pro'); ?> </th>
                    <th><?= __('Payment Mode', 'binary-mlm-pro'); ?> </th>
                    <th><?= __('Payment Detail', 'binary-mlm-pro'); ?></th></tr>
                <tr><td><?= $detail['withdrawal_initiated_date'] ?></td>
                    <td><?= $detail['payment_processed_date']; ?></td>
                    <td><?= $detail['payment_mode'] ?></td>
                    <td><?= __('Cheque No', 'binary-mlm-pro'); ?>:&nbsp;<?= $detail['cheque_no'] ?><br/>
                        <?= __('Cheque Date', 'binary-mlm-pro'); ?>: &nbsp;<?= $detail['cheque_date'] ?><br/>
            <?= __('Bank Name', 'binary-mlm-pro'); ?>:&nbsp;<?= $detail['bank_name'] ?><br/></td></tr></table>
        <?php
        }
        else
        if ($detail['payment_mode'] == 'bank-transfer')
        {
            ...
            <table><tr>
                ...
                <tr><td><?= $detail['withdrawal_initiated_date']; ?></td>
                    <td><?= $detail['payment_processed_date']; ?></td>
                    <td><?= $detail['payment_mode']; ?></td>
                    <td><?= __('Benificiary', 'binary-mlm-pro'); ?>:&nbsp;<?= $detail['beneficiary']; ?><br/>
            <?= __('Account No', 'binary-mlm-pro'); ?>: &nbsp;<?= $detail['user_bank_account_no']; ?><br/>
            <?= __('Banktransfer Code', 'binary-mlm-pro'); ?>: &nbsp;<?= $detail['banktransfer_code']; ?><br/>
            <?= __('Bank Name', 'binary-mlm-pro'); ?>:&nbsp;<?= $detail['user_bank_name']; ?> <br/></td></tr></table>
        <?php
        }
        else if ($detail['payment_mode'] == 'cash')
        {
            ...
            <table><tr>
                    <th><?= __('Withdrawal Date', 'binary-mlm-pro'); ?></th>
                    <th><?= __('Payment Date', 'binary-mlm-pro'); ?> </th>
                    <th><?= __('Payment Mode', 'binary-mlm-pro'); ?> </th></tr>
                <tr><td><?= $detail['withdrawal_initiated_date']; ?></td>
                    <td><?= $detail['payment_processed_date']; ?></td>
                    <td><?= $detail['payment_mode']; ?></td></tr></table>
        <?php
        }
        else
        if ($detail['payment_mode'] == 'other')
        {
            ...
            <table><tr>
                ...
                <tr><td><?= $detail['withdrawal_initiated_date']; ?></td>
                    <td><?= $detail['payment_processed_date']; ?></td>
                    <td><?= $detail['payment_mode']; ?></td>
                    <td><?= $detail['other_comments']; ?></td></tr></table>
        <?php
        }
    }
}
